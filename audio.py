...
class audio():
    ...
    def getSample(mic, seconds):
        ...
        def print_sound(indata, outdata, frames, time, status):
            ...
            # normalize data while adding
            num.append(int(volume_norm) - mic.threshold)
    ...
    def ready(mic, sample):  # show readiness
        ...
        for i in range(len(sample)):
            if sample[i] <= mic.threshold:
                count += 1
        # if 75% of microphone low return true
        return (count <= (len(sample) * 0.75))
    ...
    def merge(mic, files, outfile):
        ...
        if len(files) > 0:  # recursively merge all files
            newAr = [outfile] + files  # create a new array with all to add
    ...
    def recordWhileActive(mic):
        ...
        while sound:  # loop while sound is present
            ...
            b = mic.getSample(1)  # run sound checker concurrently
            b = mic.getVolume(b)
            print(b)
